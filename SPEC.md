...
- **rev-17 (registry closure for the distributed Hive)** — names the §13.1 document container
  ...
  byte-for-byte retention of persisted ones, and adds the `lifecycle` declared entry: estate-signed,
  chained notices that an organism is active, deprecated, superseded (with `superseded_by`), or archived
  since a given time (§13.5). No frozen form (§12) changes: every rev-16 `rapp/1` frame, egg, rappid, and
