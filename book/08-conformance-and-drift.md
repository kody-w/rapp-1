...
A specification you cannot test is a wish. This chapter is where RAPP/1 stops being a document
...
```
RAPP/1 rev-5 — conformance vectors
...
RAPP/1-conformant when it produces and rejects exactly these bytes. Green here is not a clean
...
RAPP/1 *reproduce* what reality stored, and does reality *conform* to the RAPP/1 envelope.
...
── rapp-body  (29 committed frames) ──
   ...
   real chain links per RAPP/1 §7.4 (prev=parent): 29/29 frames
── twin  (3 committed frames) ──
   canonicalization reproduces real stored hash : 3/3 frames
   real chain links per RAPP/1 §7.4 (prev=parent): 3/3 frames
```
...
those frames agree, because both are JCS and JCS has one answer. The parts of RAPP/1 that describe
...
```
   frames conformant to RAPP/1 §7 envelope as-is : 0/29   (rapp-body)
   frames conformant to RAPP/1 §7 envelope as-is : 0/3    (twin)
...
committed envelope uses `twin_id` where RAPP/1 has `stream_id`, `ts` where RAPP/1 has `utc`,
`sha256` where RAPP/1 has `payload_hash`, `parent_sha` where RAPP/1 has `prev` — and it carries no
...
> RAPP/1's canonicalizer reproduces the real committed payload hashes **byte-for-byte** — the spec
> matches reality exactly where reality already content-addresses. RAPP/1 then **refuses** every
...
passes when it is blind is a lie. RAPP/1's gate looks, or it fails.
