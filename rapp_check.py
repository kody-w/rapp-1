...
_SNIFF_MARKERS = (b'"rapp/1-registry"',)


def _oversized_schema(path, size, budget):
    """The top-level `schema` an over-§4 JSON file claims, so a registry that grew past 1 MiB is
    reported rather than skipped; None for anything else. Cheap for ordinary data files: only a file
    whose first or last 64 KiB names the registry schema (a canonical document sorts `schema` near its
    end) is parsed, only up to 8 MiB, and only while `budget` (a one-item list of remaining bytes, kept
    apart from frame discovery's) allows. Best effort, never a finding by itself."""
    try:
        info = os.lstat(path)
        if not stat.S_ISREG(info.st_mode):
            return None
...
def check_repo(root, signature_verifier=None):
    ...
    for path in (
        ...
        if size > R.MAX_CANONICAL_BYTES:
            ...
            # registry that grew past the limit is reported, not skipped.
