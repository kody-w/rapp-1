...
| **[`rapp_registry.py`](rapp_registry.py)** | the §13 registry as executable checks, stdlib only — exact entry shapes, kind↔family↔stream binding, owner succession, tombstones, declared entries, lifecycle notices; `examples/07_your_own_estate.py` drives it |
...
| **[`rapp_check.py`](rapp_check.py)** | bounded, stdlib-only repository linter for identities, Frames, chains, discovery indexes, eggs, and registry documents |
...
declared entries — lifecycle notices — so an estate can retire organisms and repositories
authoritatively, without changing any frozen form.
