...
| **[`rapp_registry.py`](rapp_registry.py)** | the §13 registry as executable checks, stdlib only — exact entry shapes, kind↔family↔stream binding, owner succession, tombstones, declared entries, release pins and verified snapshots; `examples/07_your_own_estate.py` and `examples/09_release_pin.py` drive it |
...
| **`examples/`** | runnable, tutorial-paced programs from first frame through typed addresses, failure steps, deterministic eggs, and the RAPP Work lifecycle |
...
declared entries — release pins — so an estate can pin every component of an LTS release,
