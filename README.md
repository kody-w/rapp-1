...
real public repos of a live estate and runs RAPP/1 against **32 actually-committed frames** — and
...
| file | what it is |
|------|-----------|
| **[`SPEC.md`](SPEC.md)** | the normative RAPP/1 standard (rev-5) — 15 sections, RFC-grounded |
| **[`rapp1.py`](rapp1.py)** | the reference implementation, stdlib only, ~140 lines you are meant to read |
| **[`conformance.py`](conformance.py)** | test vectors V1–V9; run it, watch it go green |
| **[`realcheck.py`](realcheck.py)** | runs RAPP/1 against the live estate's real committed artifacts |
...
0. **[Preface](book/00-preface.md)** — what RAPP/1 is and how to read this
...
single living standard, git making the hash the name. **RAPP/1 is the convergence: one spec, one
...
RAPP/1 rev-5. Reference implementation passes 18/18 conformance vectors and reproduces 32/32 real
