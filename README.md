...
python3 conformance.py     # V1–V29 — the spec exercised against its own claims
...
| **[`conformance.py`](conformance.py)** | test vectors V1–V9 (primitives), V10–V14 (§7.7 growth), V15–V20 (§7.8 weight), V21–V24 (§7.9 stats), V25–V29 (§7.10 cards); run it, watch it go green |
...
| **`vectors/rappid-card/`** | deterministic calling/debug card deck, generator diff-check, canonical physical frame/URI, and thread/process SQLite contention gate |
...
Ed25519 JWS. A trusted key still cannot issue a card: a signed, sequenced authority view must bind
that key to the exact subject or an explicit time-scoped `card-issuer` delegation. Runtime facts,
classification/scope, and accepted profile come from a signed runtime policy; revocation comes
from a signed, fresh, anti-rollback view covering manifest/key/subject.

Endpoint origin is signed and allow-listed; every fetch/redirect URL and DNS result is revalidated.
Replay state is transactional SQLite: `hydrating` commits before bytes are touched and `awake`
commits before success returns, across restarts, threads, and processes. The test profile remains
...
implementation passes all protocol conformance vectors, V1–V29. See
