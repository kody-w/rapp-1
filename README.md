...
to pass; (3) run python3 work_conformance.py — I expect every RAPP Work vector to
pass; (4) run python3 realcheck.py — the spec against the live public estate — and
explain the verdict in plain English; (5) run python3 examples/01_hello_frame.py and
...
python3 work_conformance.py  # organization, Hive high-water, migration, custody
...
| **[`rapp_registry.py`](rapp_registry.py)** | the §13 registry as executable checks, stdlib only — exact entry shapes, kind↔family↔stream binding, owner succession, tombstones, declared entries, release pins and verified snapshots, lifecycle notices, stream signers; `examples/07_your_own_estate.py` and `examples/09_distributed_hive_lts.py` drive it |
...
| **[`rapp_work.py`](rapp_work.py)** | stdlib reference for the seven closed `rapp-work/1` payload kinds, registry adoption, high-water, migration, receipts, observations, and rollback |
| **[`work_conformance.py`](work_conformance.py)** | positive and adversarial RAPP Work vectors, including the exact eleven-key wire |
| **[`registry_conformance.py`](registry_conformance.py)** | every §13 registry test module plus the language-neutral `conformance/registry-vectors.json`, run against `rapp_registry.py` |
| **[`rapp_check.py`](rapp_check.py)** | bounded, stdlib-only repository linter for identities, Frames, chains, discovery indexes, eggs, registry documents, and release manifests |
...
| **`examples/`** | runnable, tutorial-paced programs from first frame through typed addresses, failure steps, deterministic eggs, the RAPP Work lifecycle, and a distributed Hive's LTS |
...
### Repository Frame discovery

`rapp_check.py` does not require a Frame to live at `frames/<number>.json`.
Its bounded scan ignores `.git` and symlinks, reads regular JSON files only
within the RAPP 1 MiB JSON ceiling, and recognizes a nonstandard path only when
the top-level object has exactly the frozen eleven RAPP/1 Frame keys with
`spec:"rapp/1"`. Numeric `frames/*.json` stores remain explicit candidates, so
malformed or duplicate-key entries are findings rather than silent skips.

A producer that wants explicit discovery may place `rapp-frame-index.json`
beside its Frame files:

```json
{
  "schema": "rapp-frame-index/1",
  "stream_id": "rappid:@owner/agent:<64 lowercase hex>",
  "frames": ["genesis-frame.json", "publish/publish-frame.json"],
  "head": {"seq": 1, "frame_hash": "<64 lowercase hex>"}
}
```

Index paths are relative, regular, non-symlink JSON files. The index is a
discovery aid, not an authority or signature claim. The checker threads
discovered Frames by `stream_id` and `seq`, refuses gaps, duplicate positions,
forks, mixed indexed/numeric streams, and rollback-shaped indexed heads. A
signed chain without a caller-supplied trusted signature verifier is reported
as a `DRIFT` finding with `status:"unverified"` (exit 1), never as signed
conformance or `CLEAN`.
...
`rapp_registry.load_document(rapp_registry.parse_document(octets), trust_anchor=<that rappid>, verification_utc=<now, the first time you see it>)`
— it reports `verified` only after the §10 signature checks against that rappid, and once the registry carries
declared entries (§13.4) it needs that first-seen time, or `first_seen=` from what you persisted. This repository stays the protocol
...
content-address values, discover additive profiles, and lint public repos for compliance,
all drivable in plain English.
...
## Planetary production and portable work without freezing innovation
...
[`RAPP Work`](protocols/rapp-work/1/SPEC.md) adds one organization body stream
with seven registered kinds for discovery-only catalogs, authenticated Hive
vectors, create-only migration, signed receipts, bounded observations, and
immutable rollback. Portable Neurons remain inert typed data; Git, raw, and
static paths remain locators or evidence. The RAPP/1 frame still has exactly
eleven keys and the only wire door remains `POST /chat`.
...
specification chain/materialized view, runs every example, and enforces byte
