...
python3 conformance.py     # controlled protocol and refusal vectors
...
python3 alignment_check.py --live  # local views and observed canonical-main freshness
python3 realcheck.py       # committed artifacts in five named reference repositories
python3 examples/01_hello_frame.py   # build and verify your first frame
```

`conformance.py` exercises the reference implementation with controlled protocol
and refusal checks, including independent numeric and boundary cases. `realcheck.py`
synchronizes five named public reference repositories (`twin`, `rapp-body`, `rapp-commons`,
`rapp-map`, and `RAR`) and checks their committed numbered frame directories and
`rappid.json` records. It reads immutable Git objects at recorded commits, not dirty or
untracked working files. Drift exits nonzero; missing or unmeasurable observations also
fail. This scope is not every public repository or every kind of artifact. The captured
...
[`ALIGNMENT.md`](ALIGNMENT.md) separates local integrity, current-main observation,
captured-consumer evidence, and owner ratification. `estate_inventory.py` inventories
a complete frozen organism capture without executing its applications; it does not
turn old pins, historical examples, or static source matches into runtime compatibility.
...
| **[`rapp.py`](rapp.py)** | the stdlib-only reference implementation of the protocol |
| **[`conformance.py`](conformance.py)** | controlled protocol, boundary, and refusal vectors |
...
| **[`realcheck.py`](realcheck.py)** | fail-closed observation of explicitly selected reference-estate Git commits |
| **[`alignment_check.py`](alignment_check.py)** | verifies chain-bound local views and optionally observes current canonical main |
| **[`estate_inventory.py`](estate_inventory.py)** | evidence and coverage matrix for a frozen public-organism capture |
...
## Related carriers and tools

Public companion projects include [rapp-mission](https://github.com/kody-w/rapp-mission),
[vbrainstem](https://github.com/kody-w/vbrainstem), and
[rapp-skills](https://github.com/kody-w/rapp-skills). These are discovery links,
not protocol authority or a claim that every host executes the same behavior.
The [external profile pointers](protocols/README.md#external-profile-discovery)
identify exact specification bytes separately from adoption and runtime evidence.
...
content-address values, and inspect a public repo's `main/rappid.json` identity
record, all drivable in plain English. That SDK action is not a repository-wide
audit; use `rapp_check.py` or `realcheck.py` for their explicit artifact scopes.
...
The accepted rev-15 wire freeze keeps RAPP/1's verified forms fixed; compatible
implementation repairs do not rewrite historical artifacts. Rev-14 made
selected `anchor/chain.jsonl` frames carry normative content,
...
reference profile is exercised by the controlled core checks, independent
regressions, and operational safety vectors on every push. CI runs the suite
on Python 3.9 and 3.13, verifies the specification chain/materialized view,
runs all seven examples, and enforces transitive primitive/constant parity
between `rapp.py` and the SDK agent's embedded implementation
