...
python3 conformance.py     # V1–V24 — the spec exercised against its own claims
...
| **[`conformance.py`](conformance.py)** | test vectors V1–V9 (primitives), V10–V14 (§7.7 growth), V15–V20 (§7.8 weight), V21–V24 (§7.9 stats); run it, watch it go green |
...
| **`examples/`** | runnable, tutorial-paced programs (`01_hello_frame`, `02_build_a_chain`, `03_identity`, `04_grow_a_dimension`) |
...
5. **[The Frame](book/05-the-frame.md)** — one record that is both particle and wave; how an
   organism grows dimensions inside one unchanging name, what it weighs, and the card it can prove
...
RAPP rev-5, plus the §7.7 dimensional-growth, §7.8 weight, and §7.9 stat addenda (additive: two
registered `body` kinds; the eleven-key envelope and the `rapp/1` token unchanged). The reference
implementation passes
all protocol conformance vectors, V1–V24. See [`REAL-WORLD-REPORT.txt`](REAL-WORLD-REPORT.txt) for
the current live conformance ledger.
