...
def load_document(doc, *, trust_anchor, entries_member=ENTRIES_MEMBER, allow_unsigned=False,
    ...
    records its status in `registry.status` ("verified" or "draft"): `declared_entry_ok` checks
    it, so only a "verified" registry's copy checks (§13.4) are the estate's (a draft only with
