#!/usr/bin/env python3
"""Reference payload validator for the rapp-hive/1 operational profile."""
...
def _validate_source_frame(value: dict, where: str) -> tuple[str, str]:
    ...
    positive_int(value["seq"], f"{where}.seq", allow_zero=True)
...
def _validate_sources_and_mutations(payload: dict, where: str) -> None:
    ...
    mutation_keys = unique_strings(payload["mutation_keys"], f"{where}.mutation_keys")
    require(mutation_keys == sorted(mutation_keys), f"{where}.mutation_keys: must be sorted")
...
def validate_godd_slice(payload: dict, declaration: dict) -> str:
    ...
    positive_int(content["plaintext_bytes"], "godd slice.content.plaintext_bytes", allow_zero=True)
    positive_int(content["record_count"], "godd slice.content.record_count", allow_zero=True)
...
def _validate_candidate(candidate: dict, where: str) -> tuple[str, str]:
    ...
    positive_int(candidate["seq"], f"{where}.seq", allow_zero=True)
    ...
    mutation_keys = unique_strings(candidate["mutation_keys"], f"{where}.mutation_keys")
    require(mutation_keys == sorted(mutation_keys), f"{where}.mutation_keys: must be sorted")
...
def validate_convergence(payload: dict, declaration: dict) -> str:
    ...
    for index, candidate in enumerate(candidates):
        require(
            set(candidate["source_channel_ids"]) <= channel_ids,
            f"convergence.candidates[{index}]: unknown source channel",
        )
    ...
    for index, resolution in enumerate(resolutions):
        ...
        mutation_key = text(resolution["mutation_key"], f"{where}.mutation_key", maximum=512)
        ...
        require(set(checked_refs) <= set(frame_hashes), f"{where}.frame_hashes: unknown candidate")
        ...
        resolution_keys.append(mutation_key)
        resolution_by_key[mutation_key] = resolution
    require(
    ...
    for candidate in candidates:
        for mutation_key in candidate["mutation_keys"]:
            by_mutation.setdefault(mutation_key, []).append(candidate)
    for mutation_key, group in by_mutation.items():
        distinct_payloads = {candidate["payload_hash"] for candidate in group}
        if len(group) < 2 or len(distinct_payloads) < 2:
            continue
        group_hashes = sorted(candidate["frame_hash"] for candidate in group)
        resolution = resolution_by_key.get(mutation_key)
        if resolution is None:
            require(
                all(decision_by_hash[value]["status"] == "conflict" for value in group_hashes),
                f"convergence: unresolved mutation conflict {mutation_key!r} must preserve every branch",
            )
            continue
        require(
            resolution["frame_hashes"] == group_hashes,
            f"convergence: resolution for {mutation_key!r} must name every conflicting frame",
        )
        resolver = resolution["resolution_frame_hash"]
        for frame_hash in group_hashes:
            if frame_hash == resolver:
                continue
            require(
                decision_by_hash[frame_hash]["status"] in {"superseded", "duplicate"},
                f"convergence: resolved source {frame_hash} must be superseded or duplicate",
            )
...
def validate_projection(payload: dict, declaration: dict, convergence: dict) -> str:
    ...
    positive_int(payload["registry_seq"], "projection.registry_seq", allow_zero=True)
...
VALIDATORS = {
    DECLARATION_SCHEMA: validate_declaration,
}
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({"status": "payload-conformant", "payload_hash": result}, sort_keys=True))
    return 0
